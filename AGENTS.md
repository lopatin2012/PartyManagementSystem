...
- **Tests: `app_uip/tests.py` contains the only real test suite** (Django `TestCase`; `python manage.py test app_uip`); all other apps' `tests.py` are empty stubs. Run the suite after touching `app_uip` or anything it imports (service/view/serializer changes). Tests need a reachable PostgreSQL (Django creates/drops a temp test DB) and mock CZ/HTTP calls with `unittest.mock.patch` — no network or SUZ account required.
