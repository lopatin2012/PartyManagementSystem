...
- **`config/.env`** is the single config source (loaded by `python-dotenv` in `config/settings.py`). Never read env vars outside settings. It is gitignored and there is no `.env.example` — a fresh checkout must create it.
...
- **`docs/`** (tracked in git, despite `docs/README.md` claiming it is gitignored) is the Russian documentation set (`01-overview`…`10-presentation-outline`, `CHANGELOG`). It is derived prose — `AGENTS.md`, `README.md`, and the code are the source of truth; treat `docs/` claims (e.g. the stale Docker CMD) as unverified.
...
- **Docker** (`docker-compose.yml`: web + Postgres 15) is available but not the primary dev flow. The container CMD is `makemigrations` → `migrate` **on both DBs** (`--database archive` too) → `collectstatic` → `run_all.py`; it does **not** run `reset_nk_sync` (the `docs/06-operations.md` claim that it does is stale). The Dockerfile patches `settings.py` to take `DB_HOST` from env via `sed` and strips `pywin32`/`git+` lines from the UTF-16 requirements — don't mirror those hacks in local code.
