...
- Адреса рассылки хранятся в модели `NotificationRecipient` (email + группа);
  управляются в админке. Группа-тег «Мониторинг» создаётся автоматически после
  `migrate` (как «Админ»/«Просмотр»).
