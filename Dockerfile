...
CMD ["sh", "-c", "python manage.py makemigrations && python manage.py migrate && python manage.py migrate --database archive && python manage.py collectstatic --noinput && python run_all.py"]