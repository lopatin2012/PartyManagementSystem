...
# ==========================================
# Синхронизация справочников (Молвест.Маркировка → СУП).
# ==========================================

@task(queue_name='default')
def sync_molvest_reference_task() -> dict:
    """
    Периодическая синхронизация цехов, линий и продуктов (раз в сутки).

    Добавляет недостающие справочные данные в СУП и возвращает uuid созданных
    объектов обратно в Molvest.Маркировка (заполняется uuid_str).
    """
    from app_factory.services.molvest_reference_sync import sync_molvest_reference

    return sync_molvest_reference()
...
@task(queue_name='default')
def archive_old_codes_task() -> dict:
    """
    Периодическая архивация кодов маркировки старше 45 дней (раз в сутки).

    Переносит устаревшие коды из рабочей таблицы в архивную базу ('archive').
    Историчность данных сохраняется, безвозвратное удаление не выполняется.
    """
    from app_cz.services.code_archive import archive_old_codes

    return archive_old_codes()
...
@task(queue_name='default')
def register_reserved_uips_task() -> dict:
    """
    Регистрация зарезервированных УИП через отчёт о нанесении (раз в сутки).

    Отчёт о нанесении отправляется только для УИП, близких к сгоранию
    (до сгорания осталось менее 3 дней), у которых есть производственная
    партия со сроком годности. Код для отчёта запрашивается у внешнего
    сервиса «Молвест.Маркировка» (get_any_code_for_task). УИП без
    производственной партии со сроком годности пропускаются.
    """
    from app_cz.services.reserve_monitor import register_eligible_reserved_uips
